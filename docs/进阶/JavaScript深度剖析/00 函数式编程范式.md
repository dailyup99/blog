...
<img src="http://139.196.79.103:9001/myimages/imgs/202408040219562.png" alt="image-20240804021928409" style="zoom:67%;" />
...
![image-20240804021957327](http://139.196.79.103:9001/myimages/imgs/202408040219372.png)
...
![image-20240804022023438](http://139.196.79.103:9001/myimages/imgs/202408040220493.png)
...
使用组合函数获取数组的最后一个元素，先反转，然后再获取第1个元素，因为是从右到左执行，所以reverse写后面
...
const f = _.flowRight(toUpper, first, reverse) // 获取数组的最后一个元素，并且转为大写
...
上面实现的将NEVER SAY DIE转换为never-say-die，里面使用的方法要一个个进行柯里化，写法比较麻烦。

使用lodash的fp模块，它可以自动柯里化，并且函数优先，数据之后；lodash的join或者map等方法都是数据优先，函数之后。
...
需要定义一些辅助的基本运算函数**Point Free：**&#8203;我们可以把数据处理的过程定义成与数据无关的合成运算，不需要用到代表数据的那个参数，只要把简单的运算步骤合成到一起，在使用这种模式之前我们需要定义一些辅助的基本运算函数。
...
<img src="http://139.196.79.103:9001/myimages/imgs/202408041140049.png" alt="image-20240804114016949" style="zoom:50%;" />
