...
delete objProxy.name // 触发deleteProperty捕获器

console.log("age" in objProxy) // 触发has捕获器
```
...
const fooProxy = new Proxy(foo, {
    apply: function(target, thisArg, otherArgs) {
        ...
        target.apply(thisArg, otherArgs) // String {'abc'} [111, 222]
    },
    construct: function(target, otherArray) {
        console.log("监听执行了new操作")
        console.log(target, otherArray) // foo函数对象 ['aaa', 'bbb']
...
// Reflect.defineProperty() 严格模式下不会报错

// 1.用以前的方式进行操作
// delete obj.name 严格模式下会报错，上面已经定义不可删除
// if (obj.name) {
...
const obj = {
    ...
    set name(newValue) {
        ...
        // objProxy._name = newValue 当传递receiver过来的时候，this指向objProxy，才能监听到_name
        this._name = newValue
    },
    get name() {
        return this._name
...
const objProxy = new Proxy(obj, {
    set: function(target, key, newValue, receiver) {
        ...
        /*
           3.好处三:
             > receiver就是外层Proxy对象 receiver === objProxy 
...
当传入receiver改变this指向，this变成了objProxy，get和set都会打印两次，一次是name的，另外一次是_name。
...
function Student(name, age) {
    ...
    // 创建出来还是Student类型，但是执行的是Person中的代码
