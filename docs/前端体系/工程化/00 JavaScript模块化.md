...
在网页开发的早期，Brendan Eich 开发 JavaScript 仅仅作为一种脚本语言，做一些简单的表单验证或动画实现等，那个时候代
...
**我们需要知道 CommonJS 是一个规范，最初提出来是在浏览器以外的地方使用，并且当时被命名为 ServerJS，后来为了体现它**

**的广泛性，修改为 CommonJS，平时我们也会简称为 CJS。**
...
![image-20230315232624254](../../images/image-20230315232624254.png)
...
默认 module.exports 对象是指向 exports 对象的，但是采用了第 3 种写法之后，module.exports 会指向一个新的对象，执行 exports.name = "哈哈哈哈" ，打印 foo.name 并不会发生改变。
...
首先，是找不到 why 的，那么就会去 node_modules 目录下去查找，找到 why/index.js；

axios 也是，我们可以通过 npm install axios 命令在当前目录安装 axios，它的本质是在 nodu_modules 目录下会有 axios/index.js，其他文件是 axios 对它们的引用。

![image-20230319135140459](../../images/image-20230319135140459.png)

如果在当前目录 04_CommonJS_03 目录的 node_modules 目录查找，找不到的话，会去上一层目录的 node_modules 继续查找，直到找到根目录为止。
...
​ ![image-20230319141101850](../../images/image-20230319141101850.png)
...
在早期为了可以**在浏览器中使用模块化，通常会采用 AMD 或 CMD**：
...
**JavaScript 没有模块化一直是它的痛点，所以才会产生我们前面学习的社区规范：CommonJS、AMD、CMD 等，所以在**

**ECMA 推出自己的模块化系统时，大家也是兴奋异常。**
...
![image-20230323224108689](../../images/image-20230323224108689.png)
...
utils 文件夹下有很多工具函数，可以在 main.js 中引入使用，但是为了统一接口规范，可以将这些工具函数在 untils/index.js 中引入，再统一导出。
...
对 utils/index.js 进行优化，使用 export 结合 import，这种写法可阅读性较强
...
![image-20230327204758975](../../images/image-20230327204758975.png)
...
![image-20230327204859639](../../images/image-20230327204859639.png)
