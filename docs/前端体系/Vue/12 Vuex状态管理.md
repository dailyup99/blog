...
![image-20230529020136111](../../images/image-20230529020136111.png)
...
在 app 中通过插件安装，就是 app.use(store)；
...
context 是一个和 store 实例均有相同方法和属性的 context 对象，但是不等于 store；

所以我们可以从其中获取到 commit 方法来提交一个 mutation，或者通过 context.state 和 context.getters 来获取 state 和
...
![image-20230530144322979](../../images/image-20230530144322979.png)
...
开启了 namespaced 为 true 之后，就不需要担心命名冲突的问题
...
![image-20230530161605352](../../images/image-20230530161605352.png)

派发给根组件需要加{ root: true }，第 2 个参数 null，表示传递给根组件的数据。

这里也解释了为什么 context 对象和 store 对象不是同一个对象，而只是具有相同的方法和属性，在不同的地方 context 代表的含义不一样。
