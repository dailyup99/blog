...
React框架本身并没有提供任何动画相关的API，所以在React中使用过渡动画我们需要使用一个第三方库 react-transition
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230603065243061.png" alt="image-20230603065243061" style="zoom:80%;" />
...
![image-20230603075441636](http://139.196.79.103:9001/myimages/imgs/image-20230603075441636.png)
...
.why-leave-active {
  ...
  animation: whyAnim 2s ease reverse; // 使用whyAnim动画反过来
}
...
<template>
  <div class="app">
    ...
    <transition name="why" type="transition"> // animation动画是10秒，transition动画是2秒
      <h2 v-if="isShow">
        要是有些事我没说，地坛，你别以为是我忘了，我什么也没忘，但是有些事只适合收藏。不能说，也不能想，却又不能忘。它们不能变成语言，它们无法变成语言，一旦变成语言就不再是它们了。它们是一片朦胧的温馨与寂寥，是一片成熟的希望与绝望，它们的领地只有两处：心与坟墓。比如说邮票，有些是用于寄信的，有些仅仅是为了收藏。
      </h2>
    </transition>
...
.why-enter-active { // 这俩时间不一致，只是为了测试设置type来监听类型，开发中一般不设置type，animation和transition时间最好一样，这样就不需要设置type
...
![image-20230603081855400](http://139.196.79.103:9001/myimages/imgs/image-20230603081855400.png)

## **过渡的模式mode**

**我们来看当前的动画在两个元素之间切换的时候存在的问题：**

<img src="http://139.196.79.103:9001/myimages/imgs/image-20230603102208360.png" alt="image-20230603102208360" style="zoom:80%;" />
...
```javascript
<transition-group tag="div" name="why"> // 指定tag为div，就会有一个div包裹
    <template v-for="item in nums" :key="item"> // 需要有唯一的key
        <span>{{ item }}</span>
	</template>
</transition-group>
```
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230603104716918.png" alt="image-20230603104716918" style="zoom:80%;" />
...
span {
  margin-right: 10px;
  display: inline-block; // 这里必须改为行内块元素使用translateY才会生效
}
...
.why-leave-active {
  position: absolute; // 这里必须设置为定位元素，在删除数字时才有动画
}

</style>
```

上面为什么需要给要离开的元素加position为absolute，见下图

<img src="http://139.196.79.103:9001/myimages/imgs/image-20230603105249691.png" alt="image-20230603105249691" style="zoom:67%;" />

删除数字6，本来7 8 9是要向左移过来的，但是因为6还占据着位置，导致无法移动过来。
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230603105439847.png" alt="image-20230603105439847" style="zoom:67%;" />
...
const shuffleNumber = () => {
  nums.value = shuffle(nums.value) // 打乱数字可以用underscore这个库的shuffle方法
}
