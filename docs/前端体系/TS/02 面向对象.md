...
在早期的 JavaScript 开发中（ES5）我们需要通过函数和原型链来实现类和继承，从 ES6 开始，引入了 class 关键字，可以更加方
...
class Person {
  // 成员属性: 声明成员属性
  // name: string
  // name!: string strictPropertyInitialization模式下不希望初始化，可以这么使用，一般默认都会初始化
  name = ""; // 设置默认值可以不用指定类型，会自动推导
  age: number;
...
// printPerson("abc") // 这里会报错
printPerson({ name: "kobe", age: 30, running: function () {} }); // 这里不会报错
printPerson(new Dog("旺财", 3)); // 这里不会报错

const person: Person = new Dog("果汁", 5); // 这里不会报错
```
...
// 作用: 接口被类实现，一个类可以实现多个接口，但是必须包含接口中的所有属性和方法
...
const info: IPerson = { name: "why", age: 18, height: 1.88 };
```

如果像上面这么使用，多了一个 height 属性，那么就会报错

![image-20221204143141352](../../images/image-20221204143141352.png)
...
一个索引签名的属性类型必须是 string 或者是 number，只能是其中一个。
...
// names[0]会转为names["0"]，所以属性类型还是string，符合，返回值any也符合
// const names: IIndexType = ["abc", "cba", "nba"]

// 索引签名: [index: string]: string: 会报错
// 严格字面量赋值检测: ["abc", "cba", "nba"] => Array实例 => names[0] names.forEach
// ["abc", "cba", "nba"]本质是一个Array实例，上面有很多方法，这些方法返回值是function，而不是string所以报错了
...
interface IIndexType {
  ...
  // aaa: string
  // bbb: boolean 错误的类型 boolean不是number|string其中的一个
}
