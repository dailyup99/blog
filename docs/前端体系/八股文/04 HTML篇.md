...
## 2.SEO有哪些关键点？你在日常开发中，都采取了哪些措施来进行SEO呢？
...
* `canvas` 和 `svg` 是两种在网页上绘制图形的方式，它们的主要区别如下：

  ### 1. **绘图方式**

  - **Canvas**：基于像素（位图），使用 JavaScript 逐点绘制。
  - **SVG**：基于矢量（XML 格式），使用标签描述图形。

  ### 2. **渲染机制**

  - **Canvas**：即时渲染，图像由 JavaScript 绘制后不能直接修改，需要重新绘制整个画布。
  - **SVG**：浏览器解析 XML 并渲染，支持 DOM 结构修改，无需重新绘制整个图形。

  ### 3. **性能**

  - **Canvas**：适用于大量绘制、高频率更新的场景，如游戏、数据可视化（如 WebGL）。
  - **SVG**：适用于静态或少量对象的绘制，如图表、矢量图标。

  ### 4. **交互性**

  - **Canvas**：不具备 DOM 结构，交互（如点击）需要自行检测鼠标位置。
  - **SVG**：每个元素都是 DOM 节点，支持 CSS 和 JavaScript 事件监听，交互更方便。

  ### 5. **缩放**

  - **Canvas**：缩放会导致像素化，影响清晰度。
  - **SVG**：矢量图形可无损缩放，适用于不同屏幕尺寸。

  ### 6. **使用场景**

  - **Canvas**：游戏、动画、视频处理、粒子特效、复杂图形渲染。
  - **SVG**：图表、流程图、地图、LOGO 等。
