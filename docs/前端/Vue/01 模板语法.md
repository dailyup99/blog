...
**我们在前面练习Vue的过程中，有些代码片段是需要经常写的，我们再VSCode中我们可以生成一个代码片段，方便我们快速生**
...
![image-20230501160043147](http://139.196.79.103:9001/myimages/imgs/image-20230501160043147.png)

文件—首选项—配置用户代码片段，搜索html，然后将刚才的内容粘贴进去

之后只需要按快捷键vueapp就可以快速生成代码片段
...
<body>
  ...
  <div id="app">
    ...
    <h2 v-once>
      {{ message }} // 只会渲染一次
      <span>数字: {{counter}}</span> // 这个是子节点，不会改变，只会渲染一次
    </h2>

    <h1>{{message}}</h1> // 这个message会改变
...
**默认情况下，如果我们展示的内容本身是 html的，那么vue并不会对其进行特殊的解析。**
...
只有当name和age发送改变的时候才更新数据，height发送改变并不更新数据
...
<body>
  ...
  <div id="app">
    <div>
      <button @click="switchImage">切换图片</button> // 两张图片来回切换
    </div>
...
<body>
  ...
  <div id="app">
    ...
    <!-- <h2 v-bind:style="{ color: fontColor, 'font-size': fontSize + 'px' }">哈哈哈哈</h2>
