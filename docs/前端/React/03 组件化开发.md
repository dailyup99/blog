...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230610125834182.png" alt="image-20230610125834182" style="zoom:67%;" />
...
![image-20230610150918981](http://139.196.79.103:9001/myimages/imgs/image-20230610150918981.png)
...
点击切换按钮，组件会被卸载，执行componentWillUnmount周期函数。
...
![image-20230610152840865](http://139.196.79.103:9001/myimages/imgs/image-20230610152840865.png)
...
class HelloWorld extends React.Component {
  ...
  // 4.组件的DOM被更新完成： DOM发生更新
  componentDidUpdate(prevProps, prevState, snapshot) { // snapshot拿到记录的数据
...
点击修改文本，在render方法执行之前，会先执行shouldComponentUpdate这个方法，默认是返回true，如果改成返回false，那么就不会执行render方法，也就不会触发更新。

在componentDidUpdate函数之前有个getSnapshotBeforeUpdate方法，可以用来记录一些数据，比如滚动的位置，那么在componentDidUpdate函数的第3个参数就可以拿到记录的数据。
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230610154202817.png" alt="image-20230610154202817" style="zoom:67%;" />

推荐插件安装

ES7+ React/Redux/React-Native snippets，插件搜索react就能找到
...
export class MainBanner extends Component {
  ...
  constructor(props) { // 整个constructor也可以不写
...
![image-20230610172134198](http://139.196.79.103:9001/myimages/imgs/image-20230610172134198.png)
...
![image-20230610175924267](http://139.196.79.103:9001/myimages/imgs/image-20230610175924267.png)
...
如果传入多个元素，那么props.children是一个数组，但是如果只传入一个元素，那么props.children就是一个元素
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230610181316188.png" alt="image-20230610181316188" style="zoom:67%;" />
...
React提供了一个API：Context；类似于Vue的Provide和Inject
...
Home组件没有包裹在UserContext.Provider中

<img src="http://139.196.79.103:9001/myimages/imgs/image-20230610203151802.png" alt="image-20230610203151802" style="zoom:80%;" />
...
上面事件总线监听函数的this绑定除了上面的写法还有下面2种方法：
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230611143946361.png" alt="image-20230611143946361" style="zoom:80%;" />
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230611152713596.png" alt="image-20230611152713596" style="zoom:67%;" />

当然，我们也可以在生命周期函数：

<img src="http://139.196.79.103:9001/myimages/imgs/image-20230611152733166.png" alt="image-20230611152733166" style="zoom:67%;" />

## setState一定是异步吗？（React18之前）

验证一：在setTimeout中的更新：

<img src="http://139.196.79.103:9001/myimages/imgs/image-20230611152808288.png" alt="image-20230611152808288" style="zoom:67%;" />

验证二：原生DOM事件：

<img src="http://139.196.79.103:9001/myimages/imgs/image-20230611152918145.png" alt="image-20230611152918145" style="zoom:67%;" />

其实分成两种情况：

在组件生命周期或React合成事件（就是调用方法）中，setState是异步；
...
<img src="http://139.196.79.103:9001/myimages/imgs/image-20230611153047076.png" alt="image-20230611153047076" style="zoom:67%;" />

如果希望代码可以同步会拿到，则需要执行特殊的flushSync操作：

<img src="http://139.196.79.103:9001/myimages/imgs/image-20230611153107611.png" alt="image-20230611153107611" style="zoom:67%;" />
