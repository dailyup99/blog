...
![img](http://139.196.79.103:9001/myimages/imgs/1.9d32f541.png)
...
![img](http://139.196.79.103:9001/myimages/imgs/2-1595654633394.9f5b715a.png)
...
module.exports = {
    configureWebpack: {
        output: {
            library: 'singleVue',
            libraryTarget: 'umd'// 配置为umd，就会把bootstrap、mount、unmount挂载到window.singleVue上
        },
        devServer:{
            port:10000
        }
...
registerApplication(
    'singleVue',
    async ()=>{
        await loadScript('http://localhost:10000/js/chunk-vendors.js'); // 加载子应用打包出来的脚本
        await loadScript('http://localhost:10000/js/app.js');
        return window.singleVue
    },
    location => location.pathname.startsWith('/vue') // 用户切换到/vue路径，我需要加载刚才定义，也就是会执行上面的async里面的方法
)
start();
new Vue({
  router,
...
![img](http://139.196.79.103:9001/myimages/imgs/shadow-dom.4d23a921.png)
...
![img](http://139.196.79.103:9001/myimages/imgs/4-1595654810986.1b94a808.png)
